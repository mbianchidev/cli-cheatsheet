@echo off 
:: Official documentation here: https://docs.microsoft.com/en-us/windows-server/administration/windows-commands/windows-commands

cd [location] 							-> muoversi per andare alla location
dir 									-> cosa c'è dentro a questa cartella
mkdir [nome cartella]					-> crea una cartella

telnet [indirizzo ip] [porta] 			-> testa la connessione dal nostro host verso un servizio (ip+porta) - attenzione, va abilitata come impostazione da windows
netstat -n 								-> lista le connessioni attive sul nostro host verso di lui e verso l'esterno
ping [indirizzo ip] 					-> pinga un indirizzo ip per verificare che sia raggiungibile
tracert [indirizzo ip] 					-> ping che mostra attraverso quali indirizzi passiamo per raggiungerlo, utile per capire cosa sta bloccando un ping rendendo l'host irrangiungibile
ipconfig -all 							-> mostra tutte le configurazioni di rete del localhost
ARP -A (indirizzo ip) 					-> mostra la tabella di risoluzione degli indirizzi fornitaci dai DNS, opzionale l'indirizzo ip che mostra se uno specifico indirizzo è presente nella tabella
NSLookup 								-> mostra il DNS predefinito, una volta premuto invio possiamo digitare diversi DNS name per capire se vengono risolti dal nostro DNS predefinito

ifconfig 								-> per sapere la scheda
tcpdump -i <scheda> -n host <url dest>	-> 


systeminfo 								-> mostra info sul PC

tasklist 								-> processi PID + descrizione più preciso del gestore attività
taskkill [PID] 							-> uccide processo
sc delete <service-name>				-> cancella servizio custom (solo dalla lista NON LO DISINSTALLA - per disinstallare vedi //DS//)
iisreset /restart						-> restart IIS service


C:\Windows\System32\drivers\etc         -> contiene file hosts


#######################################################################################
################################ Windows SysInternal utils ############################
#######################################################################################
...
-> deve rispondere 235 Auth success
